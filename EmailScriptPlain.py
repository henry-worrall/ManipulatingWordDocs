...
def send_mail(recipient, subject, message, mypassword):
...
with open("plain_message.txt", "r", encoding="utf-8") as reader:
    content = reader.read()
...
with open("test_data.csv") as file:  # EDIT - for your csv file
    ...
    for (
        ...
        # adapt content to be specific for each row on the data entry - EDIT according to how you want to replace text in text file
        body = content.replace(
            "SCHOOL NAME HERE", School
        )  # new object created so that template is not overwritten
        body = body.replace(
            "SCHOOL TEACHER NAME", f"{Title} {Surname}"
